...
public class clsUserController
{
	...
	public void welcome(HttpServletRequest req) 	
	{
	...
	//Direct Show Login Page or Client Login Page
		try
		{
			...
			//{
			StringBuilder sqlCompany=new StringBuilder();
			if(req.getSession().getAttribute("WebClientCode")!=null)
			{
				 sqlCompany.append("select a.strClientCode,a.strClientName from tblsetup a where a.strClientCode='"+req.getSession().getAttribute("WebClientCode").toString()+"'");

			}
			else
			{
				 sqlCompany.append("select a.strClientCode,a.strClientName from tblsetup a");

			}
				
			List listClsSetUpModel = objBaseServiceImpl.funGetList(sqlCompany, "sql");

				if(listClsSetUpModel.size()>0)
				{
					Object[] objSetUpModel=(Object[])listClsSetUpModel.get(0);
					...
					*/
					req.getSession().setAttribute("gClientCode",objSetUpModel[0].toString());
					req.getSession().setAttribute("gCompanyName",objSetUpModel[1].toString());		
					//req.getSession().setAttribute("gStartDate",startDate);
					Map <String,Object> moduleMap=new TreeMap<String, Object>();
					...
					funWebPOSPOSSelection("P01", req, moduleMap);
					//mAndV=objDirectBiller.funOpenForm(moduleMap, req);
					//mAndV= new ModelAndView("frmLogin","command", new clsUserHdBean());
...
//        	user.setStrRetire("N");
	...
	*/
	...
	public void funWebPOSPOSSelectionMaster(@RequestParam("strPOSCode") String strPOSCode,HttpServletRequest req,Map<String,Object> model)
	{
	...
	public void funWebPOSPOSSelection(@RequestParam("strPosCode") String POSCode,HttpServletRequest req,Map<String,Object> model) throws Exception
	{
		...
		try {
			...
			if(req.getSession().getAttribute("WebQrPOSCode")!=null)
			{
				POSCode=req.getSession().getAttribute("WebQrPOSCode").toString();
			}
