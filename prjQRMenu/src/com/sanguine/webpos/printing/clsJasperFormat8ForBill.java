...
public class clsJasperFormat8ForBill
{
...
//		advAmount = rs_BillHD.getString(14);
	    ...
	    String reportName = "/WEB-INF/reports/billFormat/rptBillFormat8JasperHeader.jrxml";
	    ...
	    String sql1 = "select a.strCustomerCode,b.strCustomerName,b.strCustAddress "
			+ "from " + billhd + " a,tblcustomermaster b "
			+ "where a.strCustomerCode=b.strCustomerCode "
			+ "and a.strBillNo='" + billNo + "' "
			+ "and date(a.dteBillDate)='" + billDate + "' ";
	    ResultSet rsCustomerInfo = objDatabseCon.funGetResultSet(sql1);
	    if(rsCustomerInfo.next())
	    {
		if(rsCustomerInfo.getString(1).equals(""))
		{
		 bodyJasperParameters.put("lblCustomerName","");
	         bodyJasperParameters.put("lblCustomerAddress","");
	         bodyJasperParameters.put("customerName","");
                 bodyJasperParameters.put("customerAddress","");   
		}
		else
		{
		 bodyJasperParameters.put("lblCustomerName","Customer Name  :");
	         bodyJasperParameters.put("lblCustomerAddress","Customer Address:");
	         bodyJasperParameters.put("customerName",rsCustomerInfo.getString(2));
                 bodyJasperParameters.put("customerAddress",rsCustomerInfo.getString(3));  			  
		}
	    }
...
*/		List<clsPOSBillDtl> listOfAdvanceAmt = new ArrayList<>();
		...
		bodyJasperParameters.put("listOfAdvanceAmt", listOfAdvanceAmt);
	...
	*/	headerJasperParameters.put("imgBillLogo", "");
...
//		//Add prints to main jasper
		...
		InputStream isForInnerReport = this.getClass().getClassLoader().getResourceAsStream("/WEB-INF/reports/billFormat/rptBillFormat8JasperBody.jrxml");
		...
		{
		    //Add prints to main jasper
		    InputStream isForBillSpaceReport = this.getClass().getClassLoader().getResourceAsStream("/WEB-INF/reports/billFormat/rptBillFormat8JasperBillSpace.jrxml");
	    ...
	    List<clsPOSBillDtl> listOfAdvanceAmt = new ArrayList<>();
	    String sqlAdvance = "select a.dblAdvanceAmt from tblbilladvanceamt a where a.strBillNo='" + billNo + "' "
		    + " and a.strClientCode='" + clientCode + "' and date(dteBillDate)='"+billDate+"' ";
	    rsDtlBillNos = objDatabseCon.funGetResultSet(sqlAdvance);
	    if (rsDtlBillNos.next())
	    {
		double dblAdvAmt = rsDtlBillNos.getDouble(1);
		if (clientCode.equalsIgnoreCase("240.001"))
		{
		    objBillDtl = new clsPOSBillDtl();
		    objBillDtl.setStrItemName("                       Advance Amount       " + dblAdvAmt);
		    listOfAdvanceAmt.add(objBillDtl);
		    objBillDtl = new clsPOSBillDtl();
		    double balanceAmt=dblAllBillGrandTotal - dblAdvAmt;
		    objBillDtl.setStrItemName("                    Balance Amount       " + (balanceAmt < 0 ? 0 : balanceAmt ));
		    listOfAdvanceAmt.add(objBillDtl);
		}
	    }

	    footerJasperParameters.put("listOfAdvanceAmt", listOfAdvanceAmt);
	    footerJasperParameters.put("listOfServiceVatDetail", listOfServiceVatDetail);
	    
	    InputStream isForInnerReport = this.getClass().getClassLoader().getResourceAsStream("/WEB-INF/reports/billFormat/rptBillFormat8JasperFooter.jrxml");
...
//		jf.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
//	    }
       //    objPrintingUtility.funShowJasperFile(formName, headerJaperPrint);
